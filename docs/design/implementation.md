...
- 编辑区首行起点为左侧 16pt、顶部 20pt；正文和提示 14pt，段内行间距 5pt，代码 13pt。标题段后 10pt，正文段后及列表组末尾 6pt，连续列表项不加段后距；空段落不额外加段后距。正文与代码交界沿用代码块边距。
- 间距仅作用于显示投影，不插入空行、不改草稿文本或备忘录导出；实际截图示例已移除原本用于留白的空段落。
...
2. `xcodebuild -project NotesMate.xcodeproj -scheme NotesMate -configuration Debug -derivedDataPath build/debug-icon-composer build`。
