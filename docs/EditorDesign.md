...
正式版本只使用 `draft-v1.json`，以单个带 version 的原子 JSON 信封保存段落、run、原始图片和必要输入格式。composition 中间态、Undo 栈和平台对象不序列化。最初提出的 manifest/assets 文档包没有作为最终实现采用。

正文和图片在同一个 JSON 中原子写入，避免多文件半写入。单串行写入队列和 generation 保证旧排队任务不会覆盖新草稿；保存成功／清空时取消旧防抖任务并提交新的草稿状态。

恢复只读取并校验 `draft-v1.json`：文件不存在则无草稿，文件损坏则报错并保留原文件，后续写入前先备份损坏源。2026-09-20 的 `007dc87` 曾加入发布前的 RTFD 导入；用户确认相关测试数据已清理，2026-09-24 官网分发的 v1.0 已使用 JSON，因此该兼容路径及测试已移除。RTFD 剪贴板交换不受影响。
...
NotesMate/Editor/
  ...
  Persistence/
    DraftStore.swift           JSON v1 草稿、恢复校验和损坏源保护
...
4. **阶段 3：导入与持久化。** 完成富文本归一、附件和版本化草稿，验证失败恢复和空格式段落持久化；当时加入的发布前 RTFD 草稿迁移已按 §10.2 移除。
